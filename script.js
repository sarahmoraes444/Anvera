...
(function () {
  ...
  function addToCart(item) {
    cart.push(Object.assign({ id: cartItemSeq++ }, item));
    renderCart();
    openCart();
  }
  ...
  function renderCart() {
    ...
    if (cart.length === 0) {
      cartItemsWrap.innerHTML =
        '<div class="cart-empty">Seu carrinho está vazio.<br>Descubra sua essência para começar.</div>';
    } else {
      cart.forEach((item) => {
  ...
  function flashAdded(el) {
    ...
    el._flashTimer = setTimeout(() => el.classList.remove("show"), 2200);
  }

  checkoutBtn.addEventListener("click", () => {
    if (cart.length === 0) return;
    cartItemsWrap.innerHTML =
      '<div class="cart-empty">Pedido recebido.<br>Em breve nossa equipe entrará em contato para confirmar os detalhes da sua fórmula.</div>';
    cart = [];
    cartSubtotal.textContent = formatBRL(0);
    cartBadge.hidden = true;
  });

  document.getElementById("addPerfumeToCartBtn").addEventListener("click", () => {
    const name = fragranceName.value.trim() || "Essence 01";
    const fam = FAMILIES.find((f) => f.id === state.family);
    const noteLabels = state.notes
      .map((id) => (NOTES.find((n) => n.id === id) || {}).label)
    ...
    addToCart({
      ...
      meta:
        (fam ? fam.label : "") +
        (noteLabels.length ? " · " + noteLabels.join(", ") : "") +
        " · Modelo " + state.bottle,
      price: 489.0,
    });
    flashAdded(document.getElementById("perfumeAddedFlash"));
  });

  document.getElementById("addRefillToCartBtn").addEventListener("click", () => {
    addToCart({
      name: "Refil — Midnight Blue",
      meta: "Fórmula AV-7734-BR · Alecrim, Lavanda, Cedro",
      price: 259.0,
    });
    flashAdded(document.getElementById("refillAddedFlash"));
  });
  ...
  const SEARCH_INDEX = [
    { title: "Início", anchor: "#inicio", snippet: "Você é único. Seu perfume também será.", keywords: "hero inicio home essencia unico" },
    { title: "Descubra sua essência", anchor: "#descubra", snippet: "Teste de personalidade com 10 perguntas para mapear seu perfil olfativo.", keywords: "quiz teste personalidade descobrir essencia perguntas" },
    { title: "Como funciona", anchor: "#como-funciona", snippet: "As cinco etapas da jornada Anvera: entrevista, mapeamento, tradução olfativa, criação e entrega.", keywords: "processo etapas jornada entrevista mapeamento traducao mixologia entrega" },
    { title: "Seu perfume", anchor: "#seu-perfume", snippet: "Customizador com família olfativa, notas, intensidade, frasco e tampa.", keywords: "customizador perfume familia notas intensidade frasco tampa personalizar" },
    { title: "Modelo de negócio", anchor: "#modelo", snippet: "Consultoria e frasco na primeira compra; depois, apenas o refil da sua fórmula.", keywords: "modelo negocio formula vitalicia primeira experiencia" },
    { title: "Refis", anchor: "#refis", snippet: "Solicite novamente sua fórmula exclusiva sem refazer o processo.", keywords: "refil refis minha essencia formula codigo pedido" },
    { title: "Sustentabilidade", anchor: "#sustentabilidade", snippet: "Logística reversa e ciclo de vida responsável das embalagens.", keywords: "sustentabilidade reciclagem logistica reversa embalagem" },
    { title: "Valores", anchor: "#valores", snippet: "Individualidade, inovação, exclusividade, autenticidade e sustentabilidade.", keywords: "valores individualidade inovacao exclusividade autenticidade" },
    { title: "Sobre a Anvera", anchor: "#sobre", snippet: "Psicologia, perfumaria, tecnologia e individualidade em um só propósito.", keywords: "sobre anvera historia proposito psicologia perfumaria tecnologia" },
    { title: "Privacidade e proteção de dados", anchor: "#privacidade", snippet: "Como a Anvera coleta, usa e protege seus dados, conforme a LGPD.", keywords: "privacidade lgpd dados seguranca cookies termos" },
    { title: "Trabalhe conosco", anchor: null, external: true, snippet: "Candidate-se enviando seu currículo pelo nosso formulário.", keywords: "trabalhe conosco vagas emprego curriculo candidatura formulario" },
  ];
  ...
  function renderSearchResults(query) {
    ...
    if (!q) {
      searchResults.innerHTML = '<p class="search-hint">Digite para buscar em todo o site.</p>';
      return;
    }
    ...
    if (matches.length === 0) {
      searchResults.innerHTML = '<p class="search-hint">Nada encontrado para "' + query + '".</p>';
      return;
    }
    ...
    matches.forEach((item) => {
      ...
      btn.addEventListener("click", () => {
        closeSearch();
        if (item.external) {
          window.open("https://forms.gle/REPLACE-COM-SEU-LINK-DO-FORMULARIO", "_blank", "noopener");
        } else {
  ...
  const accountLoggedView = document.getElementById("accountLoggedView");
  ...
  function renderAccount() {
    if (session) {
      ...
      accountLoggedView.classList.add("active");
      accountModalTitle.textContent = "Minha conta";
      document.getElementById("accountAvatar").textContent = initials(session.name);
      ...
      accountLoggedView.classList.remove("active");
      accountModalTitle.textContent = "Minha conta";
